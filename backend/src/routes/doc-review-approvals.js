...
router.post('/projects/:projectId/approve', authenticateToken, authorizePermission('checkpoint.approve'), async (req, res) => {
  try {
    ...
    // Check if all required documents are passed
    ...
    await pool.transaction(async (tx) => {
      ...
      await tx.query(
        `INSERT INTO doc_review_approvals (id, project_id, approver_id, approval_status, comment)
         VALUES (?, ?, ?, 'approved', ?)`,
        [id, req.params.projectId, req.user.id, comment || 'เอกสารครบถ้วน พร้อมยื่นหน่วยงาน']
      );

      // Update project status
      await tx.query(
        'UPDATE doc_review_projects SET project_status = ?, updated_at = datetime("now") WHERE id = ?',
        ['ready_to_submit', req.params.projectId]
      );
    });

    logActivity(req.user.id, 'approve', 'doc_review_project', req.params.projectId, { comment });
