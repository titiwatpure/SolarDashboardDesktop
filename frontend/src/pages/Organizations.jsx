...
export default function Organizations() {
  ...
  // สำหรับแสดงโครงการของแต่ละหน่วยงาน
  ...
  const openEditModal = (org) => {
    setEditingOrg(org);
    setFormData({ org_name: org.org_name, org_type: org.org_type });
    setIsModalOpen(true);
  };
          ...
          organizations.map(org => (
            <div key={org.id} className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                ...
                <button
                  onClick={() => toggleOrgProjects(org.id)}
                  className="inline-flex items-center gap-1.5 rounded-xl bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-600 hover:bg-blue-100 transition-colors"
                >
                  ดูโครงการ
                  <ChevronDown size={14} />
                </button>
              ...
              {/* แสดงโครงการที่เชื่อม */}
              {expandedOrgId === org.id && (
                <div className="mt-4 pt-4 border-t border-slate-100">
                  {loadingProjects ? (
                    <div className="text-center py-3">
                      <Loader size={16} className="animate-spin mx-auto text-slate-400" />
                    </div>
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto">
                      {orgProjects.map(p => (
                        <div
                          ...
                          className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 cursor-pointer hover:bg-blue-50 transition-colors"
                        >
                          <div>
          ...
          <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <h2 className="text-base font-bold text-slate-900">
                {editingOrg ? 'แก้ไขหน่วยงาน' : 'เพิ่มหน่วยงานใหม่'}
              </h2>
